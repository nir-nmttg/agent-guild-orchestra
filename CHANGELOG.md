...
- Rootの推論レベルは通常`xhigh`を推奨し、利用者の選択を優先する方針を明記。配布設定では引き続き固定しない
- 重大リスクに限定したInquisitorの推論レベルを`xhigh`から`max`へ変更し、導入案内、構造検証、名前付き起動probeの期待値を更新
- InquisitorがAstra/maxの評価条件`mixed-luna-v2`を追加。既存`mixed-luna-v1`と旧評価記録を保持し、条件ごとに分離して検証・集計する。実モデルの品質・時間・使用量の比較は未実施
