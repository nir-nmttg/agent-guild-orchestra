...
Rootはgpt-6-astraで要件、分割、難しい判断、統合を担当し、推論レベルは利用者の選択を尊重します。独立した仕事を役割に応じて直接委譲します。小さな作業はRootが直接完了できます。
...
独自設定を保持した場合は、Astraモデル、1Mコンテキスト・90万自動コンパクション、エージェントの有効化と同時実行上限8、既定のGPT-6 Luna/max、`multi_agent`、実験的コンテキスト管理の設定を手動で整合させます。Sentinelには名前付き定義のGPT-6 Sol/xhighを使います。Guildmasterの推論レベルは利用者がタスク/セッションで選びます。子のAGENTS指示はコード変更前に読み、既存の子設定・Skill・名前付きエージェントとの競合を確認します。インストーラーは該当パスを`child_overrides`へ表示し、子設定を自動統合しません。
