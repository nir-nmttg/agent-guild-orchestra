...
Agent Guild Orchestra 3.2.0は、Codexのプロジェクト固有設定、五つのカスタムエージェント、五つのSkill、安全確認用の小さな補助スクリプトを非Gitの親ディレクトリへ配布するテンプレートです。常駐サービスや独自スケジューラーはありません。Codex自身の会話履歴、サブエージェント、メッセージ、承認を使います。
...
Rootはgpt-6-astraで要件、分割、難しい判断、統合を担当します。推論レベルは通常`xhigh`を推奨し、利用者の選択を優先して配布設定では固定しません。独立した仕事を役割に応じて直接委譲します。小さな作業はRootが直接完了できます。

| カスタムエージェント | モデル・推論 | 担当 |
| --- | --- | --- |
| Scholar | GPT-6 Luna / max | コード・資料調査、影響範囲、反証。read-only |
| Adventurer | GPT-6 Luna / max | 独立した範囲の実装と局所テスト。workspace-write |
| Verifier | GPT-6 Luna / max | 要件から独立した受け入れ検証。指定テスト・生成物だけに書き込み |
| Sentinel | GPT-6 Sol / xhigh | 差分と関連箇所の整合性、回帰、検証漏れのレビュー。read-only |
...
作業に必要な独立コンテキストと要約・根拠の返却で、Rootが読む途中経過と重複作業を減らします。返却量は情報量に応じて増減し、判断を左右する条件・指摘・未検証範囲は本文に残します。分量の共通目安はAGENTS.mdで管理します。小変更では追加レビューを省き、実行結果が不足していればVerifier、整合性・影響範囲・検証漏れの確認が必要ならSentinelを選びます。異なる証拠が必要な場合だけ両方を使います。セキュリティ、インストーラーやGit安全規則、移行、広い互換性などの重大なリスクはInquisitorが確認し、同じ観点の通常レビューは重ねません。通常のローカルGit操作だけでは追加レビューを要求しません。指摘は根拠とともにRootへ返し、修正の割り当て・統合・最終受け入れはRootが担当します。重要な結論はRootが証拠を照合し、多数決だけで決めません。
...
独自設定を保持した場合は、Astraモデル、1Mコンテキスト・90万自動コンパクション、エージェントの有効化と同時実行上限8、既定のGPT-6 Luna/max、`multi_agent`、実験的コンテキスト管理の設定を手動で整合させます。Sentinelには名前付き定義のGPT-6 Sol/xhigh、InquisitorにはGPT-6 Astra/maxを使います。Guildmasterの推論レベルは通常`xhigh`を推奨し、利用者がタスク/セッションで選びます。子のAGENTS指示はコード変更前に読み、既存の子設定・Skill・名前付きエージェントとの競合を確認します。インストーラーは該当パスを`child_overrides`へ表示し、子設定を自動統合しません。

GPT-5.6 Lunaを使う旧構成では、Codex 0.153.4で親の有効設定（上限8）、5役の定義、AGENTS.md、五つのSkillの読み込みと子設定との分離を確認しました。GPT-6 Luna/Solへの移行後の名前付き役の実起動、実際の8体並列、性能改善は未検証です。名前付き役の実起動・実効権限は、設定解析と区別して[親配置の検証記録](docs/parent-layout.md)に記載します。再確認用probeでは対象役を限定できます。ファイル配置の成功だけでは実行時の有効化完了を意味しません。

## 更新

GPT-5.6 Lunaからの更新内容と独自設定の確認箇所は[GPT-6移行ガイド](docs/migration-gpt6.md)を参照してください。
