...
`[agents]`の`default_subagent_model = "gpt-5.6-luna"`と`default_subagent_reasoning_effort = "max"`を既定値にします。名前付きエージェントのTOMLに明示したモデルと推論レベルがこれらの既定値より優先されるため、レビューは`inquisitor`を指定してAstra / xhighで起動します。起動時にモデルと推論レベルを明示できるホストでは、その指定も既定値より優先されます。[公式の設定優先順位](https://learn.chatgpt.com/docs/agent-configuration/subagents)
...
AdventurerとInquisitorの各TOMLにも同じコンテキスト長100万・コンパクションしきい値90万を明示します。[カスタムエージェントの設定](https://learn.chatgpt.com/docs/agent-configuration/subagents)として読み込むため、名前付きエージェント定義を使って起動します。設定値の検証と、モデル提供側が実際に受け付けるコンテキスト長の実機確認は別であり、100万トークンの実リクエストは検証していません。
