...
| 役割 | 移行後のモデル | 推論レベル |
| --- | --- | --- |
| Guildmaster / Root | `gpt-6-astra` | 利用者の選択。配布設定では固定しない |
...
既定のsubagentは`gpt-6-luna/max`です。SentinelとInquisitorは名前付き定義を使い、名前付き役を指定できないホストでは対応するモデル・推論・役割指示を明示します。Sentinelの推論を`max`へ上げることは今回の固定構成に含めません。
...
- Rootの`model = "gpt-6-astra"`と利用者の推論レベル選択。
...
- `.codex/agents/`の5役、特に`sentinel.toml`の`gpt-6-sol/xhigh`と、各役の権限・追加エージェント起動禁止。
- 親のAGENTS管理ブロック、`design-review`と`verify-change`の指示。親の`AGENTS.override.md`、子設定、ユーザー/セッション上書きとの競合。

更新後は親を開いた新しいタスクで確認します。名前付き定義は明示した起動値や既定値より優先されるため、旧モデルを記した名前付き定義が残っていないか確認してください。[公式の設定優先順位](https://learn.chatgpt.com/docs/agent-configuration/subagents)
...
評価器には新しい固定配分`mixed-luna-v1`を追加します。旧GPT-5.6 Lunaのbaselineとfixtureは保持し、条件IDとRootのモデル・推論レベルごとに集計します。合成データは集計の検証専用です。詳細は[評価手順](model-selection-evaluation.md)を参照してください。
