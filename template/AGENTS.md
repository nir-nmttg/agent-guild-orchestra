...
- 独立した一つの範囲の実装と、それに必要な重点的な検証は、`adventurer`（`gpt-5.6-luna`、`max`、`workspace-write`）へ任せます。目的、受け入れ条件、対象、権限、担当パスを渡し、実行環境が対応していれば短い独立したコンテキストを使います。他のエージェントの起動や担当外の統合は行わせません。利用可能なCodex標準の操作で名前付きエージェント、またはモデルと推論レベルを明示して選び、メインのモデルを暗黙に継承させません。役割を選択できない場合は実行環境の制約として報告します。
- 独立した調査は、`scholar`（`gpt-5.6-luna`、`max`、`read-only`）へ任せます。コード探索、公式資料調査、比較、反例検証を担当させ、依頼の目的、対象、範囲、完了条件を渡します。短い独立コンテキストを使い、名前付き`scholar`または`gpt-5.6-luna`/`max`を明示して起動し、メインのモデルを暗黙に継承させません。結論に不利な証拠や成立しない条件も探させます。結果は結論、根拠、反証・制約、未確認点を含め、事実と推測を区別させ、完了条件を満たしたら短く返させます。編集、Git書き込み、外部更新、子エージェントの起動はさせません。
...
- Rootは独立調査を通常1体へ渡し、論点を分離できる場合だけ観点・資料を分けて最大3体まで並列します。重要な結論は独立確認・反証（2並列と逐次1など）を組み合わせ、Rootが根拠を照合して判断します。多数決だけで採否を決めません。
