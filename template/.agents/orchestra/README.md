...
メインセッションのGuildmasterは作業全体の成果に責任を持ちます。範囲の結び付きが強い作業や小さな作業は、直接行って構いません。`adventurer`は、独立した一つの範囲の実装と、それに必要な重点的な検証を担当します。`scholar`は、コード探索、公式資料調査、比較、反例検証を行う調査専用の読み取り専用担当です。`inquisitor`は重大なリスクがある時に、新しい独立したコンテキストで読み取り専用の最終レビューを行います。通常の再試行のためには使わず、他の役割も起動しません。

独立調査は通常Scholar 1体へ渡し、論点を分けられる場合だけ観点・資料を分けて最大3体まで並列します。重要な結論はRootが独立確認・反証の根拠を照合して判断し、多数決だけで採否を決めません。Scholarは編集、Git書き込み、外部更新、子エージェントの起動を行わず、`read-only`設定だけをOS上の権限保証とはみなしません。
