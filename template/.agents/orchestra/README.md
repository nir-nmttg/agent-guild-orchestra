...
短い独立コンテキストで委譲し、結論・根拠・検証・未確認点・必要な判断を短く返します。同じ探索や失敗を繰り返さず、証拠の衝突や統合の難所はRootへ返します。子の完了と枠の解放を同一視せず、標準の終了操作がなければ関連する継続作業に既存担当を使います。重大リスクの独立レビュー用の枠を確保し、独自の枠管理サービスは追加しません。
